...
Component({
  ...
  methods: {
    async payOrder(e) {
      ...
      const params = await orderService.getMealTicketPayParams(id);
      wx.requestPayment({
        ...params,
        success: () => {
    ...
    navToEvaluation(e) {
      ...
      const url = `/pages/subpages/mine/order/subpages/meal-ticket-order/evaluation/index?orderId=${orderId}&restaurantId=${restaurantId}`;
      wx.navigateTo({ url });
    },
