...
Page({
  ...
  search() {
    ...
    cateringService.saveKeywords(keywords);
    this.setOrderList();
  ...
  async setOrderList() {
    const { keywords } = this.data;
    const orderList = (await cateringService.searchOrderList(keywords)) || [];
    this.setData({ orderList });
  },

  async setHistoryKeywords() {
    const historyKeywords = await cateringService.getHistoryKeywords();
    ...
    wx.showModal({
      ...
      success: result => {
        if (result.confirm) {
          ...
          cateringService.clearHistoryKeywords();
        }
      }
    });
  },
