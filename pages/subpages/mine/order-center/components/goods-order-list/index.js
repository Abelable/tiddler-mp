...
Component({
  ...
  methods: {
    ...
    navToDetail(e) {
      ...
      const url = `/pages/subpages/mine/order-center/subpages/goods-order/order-detail/index?id=${id}`;
      wx.navigateTo({ url });
    },

    navToShipping(e) {
      const id = e.currentTarget.dataset.id;
      const url = `/pages/subpages/mine/order-center/subpages/goods-order/shipping/index?id=${id}`;
      wx.navigateTo({ url });
    },

    navToEvaluation(e) {
      const { id, goodsList } = e.currentTarget.dataset;
      const url = `/pages/subpages/mine/order-center/subpages/goods-order/evaluation/index?orderId=${id}&goodsList=${JSON.stringify(goodsList)}`;
      wx.navigateTo({ url });
    },
