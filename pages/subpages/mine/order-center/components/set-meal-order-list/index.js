...
Component({
  ...
  methods: {
    ...
    navToDetail(e) {
      ...
      const url = `/pages/subpages/mine/order-center/subpages/set-meal-order/order-detail/index?id=${id}`;
      wx.navigateTo({ url });
    },

    navToEvaluation(e) {
      const { orderId, restaurantId } = e.currentTarget.dataset;
      const url = `/pages/subpages/mine/order-center/subpages/set-meal-order/evaluation/index?orderId=${orderId}&restaurantId=${restaurantId}`;
      wx.navigateTo({ url });
    },
