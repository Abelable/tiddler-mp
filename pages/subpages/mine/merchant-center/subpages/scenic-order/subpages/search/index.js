...
Page({
  ...
  search() {
    ...
    scenicService.saveKeywords(keywords);
    this.setOrderList();
  ...
  async setOrderList() {
    const { keywords } = this.data;
    const orderList = (await scenicService.searchOrderList(keywords)) || [];
    this.setData({ orderList });
  },

  async setHistoryKeywords() {
    const historyKeywords = await scenicService.getHistoryKeywords();
    ...
    wx.showModal({
      ...
      success: result => {
        if (result.confirm) {
          ...
          scenicService.clearHistoryKeywords();
        }
      }
    });
  },
