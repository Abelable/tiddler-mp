...
class BaseService extends Base {
  ...
  async getHotelOrderTotal() {
    const total = await this.get({
      url: `${this.baseUrl}/hotel/order/total`
    });
    store.setHotelOrderTotal(total);
    return total;
  }

  async getMealTicketOrderTotal() {
    const total = await this.get({
      url: `${this.baseUrl}/catering/meal_ticket/order/total`
    });
    store.setMealTicketOrderTotal(total);
    return total;
  }
  ...
  // todo 团圆家乡年
  async finishTask(taskId, userId = undefined, referenceId = "") {
    return await this.post({
